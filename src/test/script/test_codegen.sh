...
# Function to process a single .deca file
process_file() {
    local deca_file="$1"
...
# Find and process all .deca files using a more compatible approach
for deca_file in $(find "$TEST_DIR" -type f -name "*.deca"); do
    ((total_files++))
    process_file "$deca_file"
done
